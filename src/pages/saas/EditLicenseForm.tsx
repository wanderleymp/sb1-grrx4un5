...
import { Globe, Package, Calendar, Building2, Palette, FileText, Shield, AlertCircle, Search } from 'lucide-react';
import { useMask } from '@react-input/mask';
import { licenseAPI, License } from '../../services/api/licenses';
...
export function EditLicenseForm({ license, onClose }: EditLicenseFormProps) {
  const { updateLicense } = useSaaSStore();
  ...
  const [formData, setFormData] = React.useState({
    ...
    documentType: license.document_type || 'cnpj',
    modules: license.modules,
    expiresAt: license.expires_at ? new Date(license.expires_at).toISOString().split('T')[0] : '',
    primaryColor: license.primary_color,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedLicense = await licenseAPI.update(license.id, {
      ...
      console.log('Licença atualizada com sucesso:', updatedLicense);
      ...
      setErrors(prev => ({
        ...prev,
        submit: `Erro ao atualizar licença: ${message}`
      }));
    } finally {
  ...
  return (
    <div className="bg-white rounded-xl p-6 max-w-2xl mx-auto">
      <div className="flex items-center space-x-3 mb-6">
        ...
        <h2 className="text-2xl font-bold text-gray-900">Editar Licença</h2>
      </div>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    console.error('Erro ao consultar CNPJ:', message);
                    setErrors(prev => ({
                      ...prev,
                      document: message
                    }));
